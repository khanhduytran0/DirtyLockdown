...
struct lockdownd_self_pairing_offsets {
  uint64_t offset_auth_got__getsockopt;
  uint64_t offset_just_return_0;
  bool is_arm64e;
};
...
static NSData* patchTCCD(void* executableMap, size_t executableLength) {
  ...
  if (offsets.is_arm64e && offsets.offset_auth_got__sandbox_init) {
    ...
    *(struct dyld_chained_ptr_arm64e_auth_rebase*)(mutableBytes +
        ...
        targetRebase;
  } else if (offsets.offset_auth_got__sandbox_init) {
    // make sandbox_init call return 0;
...
// MARK: - lockdownd patch

static uint64_t patchfind_getsockopt_return_0(void* executable_map, size_t executable_length) {
  // getsockopt
  // mov x0, #0
  // mov x3, #0
  // ret
  static const char needle[] = {0x00, 0x00, 0x80, 0xd2, 0x03, 0x00, 0x80, 0xd2, 0xc0, 0x03, 0x5f, 0xd6};
  void* offset = memmem(executable_map, executable_length, needle, sizeof(needle));
  if (!offset) {
    return 0;
  }
...
static bool patchfind_lockdownd(void* executable_map, size_t executable_length,
                      struct lockdownd_self_pairing_offsets* offsets) {
  struct segment_command_64* data_const_segment = nil;
  struct symtab_command* symtab_command = nil;
  struct dysymtab_command* dysymtab_command = nil;
  if (!patchfind_sections(executable_map, &data_const_segment, &symtab_command,
  ...
  if ((offsets->offset_auth_got__getsockopt =
           patchfind_got(executable_map, executable_length, data_const_segment, symtab_command,
                         dysymtab_command, "_getsockopt")) == 0) {
    NSLog(@"no getsockopt\n");
    return false;
  }
  if ((offsets->offset_just_return_0 = patchfind_getsockopt_return_0(executable_map, executable_length)) ==
      0) {
    NSLog(@"no just return 0\n");
    return false;
  }
  struct mach_header_64* executable_header = executable_map;
  offsets->is_arm64e = (executable_header->cpusubtype & ~CPU_SUBTYPE_MASK) == CPU_SUBTYPE_ARM64E;
...
static NSData* patchLockdownd(void* executableMap, size_t executableLength) {
  struct lockdownd_self_pairing_offsets offsets = {};
  if (!patchfind_lockdownd(executableMap, executableLength, &offsets)) {
    return nil;
  }
  ...
  if (offsets.is_arm64e) {
    ...
    *(struct dyld_chained_ptr_arm64e_auth_rebase*)(mutableBytes +
                                                   offsets.offset_auth_got__getsockopt) =
        targetRebase;
  } else {
    // make getsockopt call return 0;
    struct dyld_chained_ptr_64_rebase targetRebase = {
    ...
    *(struct dyld_chained_ptr_64_rebase*)(mutableBytes + offsets.offset_auth_got__getsockopt) =
        targetRebase;
  }
  return data;
}

static NSData* make_patch_lockdownd(void* executableMap, size_t executableLength) {
  struct lockdownd_self_pairing_offsets offsets = {};
  if (!patchfind_lockdownd(executableMap, executableLength, &offsets)) {
    return nil;
  }
...
bool patch_lockdownd(void (^completion)(NSString* _Nullable)) {
  const char* targetPath = "/usr/libexec/lockdownd";
  int fd = open(targetPath, O_RDONLY | O_CLOEXEC);
  off_t targetLength = lseek(fd, 0, SEEK_END);
  lseek(fd, 0, SEEK_SET);
  void* targetMap = mmap(nil, targetLength, PROT_READ, MAP_SHARED, fd, 0);

  NSLog(@"about to alloc %lld", targetLength);
  NSData* originalData = [NSData dataWithBytes:targetMap length:targetLength];
  NSData* sourceData = make_patch_lockdownd(targetMap, targetLength);
  if (!sourceData) {
    completion(@"can't patchfind");
    return false;
  }
  ...
  if (!overwrite_file(fd, sourceData)) {
    overwrite_file(fd, originalData);
    munmap(targetMap, targetLength);
    completion(@"can't overwrite");
    return false;
  }
  munmap(targetMap, targetLength);
  // FIXME: this will cause panic
  //xpc_crasher("com.apple.mobile.lockdown");
  sleep(1);

  // TODO(zhuowei): for now we revert it once lockdownd starts
  // so the change will only last until when this lockdownd exits
  // also safeguard for iOS 14, where overwriting stays permanent
  overwrite_file(fd, originalData);
  return true;
}
